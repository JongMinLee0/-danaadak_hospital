...
function displayPlaces(search) {
	...
	for (var i = 0; i < search.length; i++) {
		...
		var placePosition = new daum.maps.LatLng(
				search[i].ph_lat,
				search[i].ph_lon), marker = addMarker(
...
function getListItem(index, search, marker) {
	...
	if (search.newAddress) {
		itemStr += '<span>' + search.ph_address + '</span>'
				+ '<span class="jibun gray">' + search.ph_address
				+ '</span>';
	} else {
		itemStr += '<span>' + search.ph_address + '</span>';
	}

	itemStr += '<span class="tel">' + search.ph_phone + '</span>'
			+ '</div>';
...
function displayInfowindow(marker, title, searchs) {
    ...
    '           </div>' + 
    '            <div class="desc">' + 
    '                <div class="ellipsis">'+searchs.ph_address+'</div>' + 
    '                <div class="jibun ellipsis">'+searchs.ph_phone+'</div>' +
    '                <div><a href="'+searchs.ph_url+'" target="_blank" class="link">상세보기</a></div>' + 
    '            </div>' + 
    '        </div>' + 
    '    </div>' +    
    '</div>';
